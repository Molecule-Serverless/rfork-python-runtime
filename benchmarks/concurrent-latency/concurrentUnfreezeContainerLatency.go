...
func routine(count int, socketName string, newContainerName string, _ string) (int64, error) {
	...
	err := freezeContainer(newContainerName)
	if err != nil {
		panic(err)
	}
	...
	err = unfreezeContainer(newContainerName)
	if err != nil {
		panic(err)
	}
...
func freezeContainer(containerName string) error {
	freezeCmd := exec.Command(runc, "pause", containerName)
	err := freezeCmd.Run()
	return err
}

func unfreezeContainer(containerName string) error {
	unfreezeCmd := exec.Command(runc, "resume", containerName)
	err := unfreezeCmd.Run()
	return err
}
